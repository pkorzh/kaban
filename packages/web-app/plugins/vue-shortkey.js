...
const createShortcutIndex = (pKey) => {
	...
	if ((pKey.key && pKey.key !== ' ' && pKey.key.length === 1) || /F\d{1,2}|\//g.test(pKey.key)) {
		k += getDelocalizedLetterKey(pKey.key.toLowerCase())
	}
	return k
}

const getDelocalizedLetterKey = (key) => {
	if (/^[a-z]$/.test(key)) return key

	const eng = 'qwertyuiopasdfghjklzxcvbnm'
	const rus = 'йцукенгшщзфывапролдячсмить'
	const ukr = 'йцукенгшщзфівапролдячсмить'

	if (rus.indexOf(key) !== -1) {
		return eng[rus.indexOf(key)]
	} else if (ukr.indexOf(key !== -1)) {
		return eng[ukr.indexOf(key)]
	} else {
		return key;
	}
}
